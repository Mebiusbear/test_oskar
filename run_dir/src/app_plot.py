    ...
    for fits_name in fits_list:
        if freq in fits_name and "fits" in fits_name:
            if "errors" in fits_name:
                hdul = fits.open(fits_name)[0]
                im_list[i][0] = hdul.data[0]
            else:
                im_list[i][1] = fits.open(fits_name)[0].data[0]
    filename = list()
    filename.append(freq)
    filename.append("MHz")
    filename.append("compare")
    filename += feture
    filename = "../compare_fits_gather/" + "_".join(filename) + ".fits"

    residual = im_list[i][0] - im_list[i][1]
    RMS = np.sum(np.power(residual,2)) / np.power(residual.shape[0],2)
    hdul.header.append("RMS")
    hdul.header["RMS"] = "%.6f"%RMS

    fits.writeto(filename=filename, data=residual,\
             header=hdul.header, overwrite=True)

    

# fits.open(fits_name)[0].hearder
